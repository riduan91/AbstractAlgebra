...
class BigradedAlgebra:
    ...
    def multiplyProducts(self, product1, product2):
        product1 = self.simplifyProduct(product1)
        product2 = self.simplifyProduct(product2)
        if product1 == [] or product2 == []:
            return []
        else:
            return self.simplifyProduct(product1 + product2)
    ...
    def possibleTermsOfRank(self, rank):
        if rank <= 0:
            return [(1, [])]
        else:
            res = []
            preres = self.possibleTermsOfRank(rank - 1)
            for i in range(self.nb_generators):
                for term in preres:
                    term = self.simplifyTerm((term[0], term[1] + [i]))
                    if len(term[1]) == rank:
                        res.append(term)
            return res
    
    def possibleTermsOfRankWithLimit(self, rank, lim):
        if rank <= 0:
            return [(1, [])]
        else:
            res = []
            ...
            for i in range(self.nb_generators):
                for term in preres:
                    term = self.simplifyTerm((term[0], term[1] + [i]))
                    if len(term[1]) == rank:
                        mysum = sum([self.bidegrees[x] for x in term[1]])
                        if abs(mysum[0]) + abs(mysum[1]) <= lim:
                            res.append(term)
            return res
    
    def possibleTermsOfRankNoGreaterThan(self, rank):
        res = []
        for r in range(rank + 1):
            s = self.possibleTermsOfRank(r)
            res += s
            if len(s) == 0:
                break
        return res
    
    def possibleTermsOfRankNoGreaterThanWithLimit(self, rank, lim):
        res = []
        for r in range(rank + 1):
            s = self.possibleTermsOfRankWithLimit(r, lim)
            res += s
            if len(s) == 0:
                break
        return res
    
    def possibleBidegreesOfRankNoGreaterThan(self, rank):
        allterms = self.possibleTermsOfRankNoGreaterThan(rank)
        bidegrees = {}
    ...
    def getConnectionsByAction(self, rank, element, side):
        source_bidegrees = self.possibleBidegreesOfRankNoGreaterThan(rank)
        connections = []
    ...
    def getConnectionsByActionWithLimit(self, rank, element, side, lim):
        source_bidegrees = self.possibleBidegreesOfRankNoGreaterThanWithLimit(rank)
        connections = []
        for k in source_bidegrees.keys():
            k_as_list = map(int, k[1:-1].split(", "))
            for V in source_bidegrees[k]:
    ...
    def getConnectionsByActionFromTheLeftWithLimit(self, rank, term, lim):
        return self.getConnectionsByActionWithLimit(rank, term, 'left', lim)
    
    def getConnectionsByActionFromTheRightWithLimit(self, rank, term, lim):
        return self.getConnectionsByAction(rank, term, 'right', lim)
